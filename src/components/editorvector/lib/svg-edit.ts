...
export function deriveEditedSvg(
    ...
    editRegions: EditableRegion[] | null,
    visibleCount: number,
    ...
    const baseColorEl = doc.getElementById(visible[0].element_ids[0]);
    for (const r of hidden) {
        for (const id of r.element_ids) {
            const el = doc.getElementById(id);
            if (el) el.setAttribute("fill", baseColorEl?.getAttribute("fill") || "transparent"); // hide by matching the first visible region's fill (or transparent if none)
        }
    }

    // 2. Apply copy_color on visible regions.
    for (const r of editRegions ?? []) {
        const colorParent = r.color;
        const childs = r.children ?? [];
        for (const child of childs ? childs : []) {
            for (const id of child.element_ids) {
                const el = doc.getElementById(id);
                if (el) el.setAttribute("fill", colorParent || "transparent");
            }
        }
    }
    ...
    if (highlightedRegionId) {
        const r = editRegions?.find((x) => x.id === highlightedRegionId);
        if (r) {
            for (const id of r.element_ids) {
                const el = doc.getElementById(id);
                if (el) {
